...
import ChurchNatural.{One, Zero}

class ChurchNaturalSuite extends AnyFunSuite with Matchers {
  private def nat(i: Int): ChurchNatural =
    new ChurchNatural {
  ...
  test("fromInt is correct") {
    for (x <- List(0, 1, 3, 4, 6, 10, 23, 100))
      ChurchNatural.fromInt(x).map(_.toInt) mustBe Some(x)

    for (x <- List(-1, -4, -100))
      ChurchNatural.fromInt(x) mustBe None
  }
