  ...
  def solve: Option[Solution] = {
    ...
    while (queue.nonEmpty) {
      ...
      if (houses.exists(!_.isDefined)) {
        for {
          housesWithColor <- Color.update(houses, conditions)
          housesWithDrink <- Drink.update(housesWithColor, conditions)
          housesWithResident <- Resident.update(housesWithDrink, conditions)
          housesWithAnimal <- Animal.update(housesWithResident, conditions)
          housesWithSmoke <- Smoke.update(housesWithAnimal, conditions)
